...
<div class="container my-5">
            ...
            <div class="row g-2 align-items-center mb-3">
                <div class="col-md-3">
                    <select name="dateType" class="form-select">
                        <option value="postLikeCreatedAt" ${param.dateType eq 'postLikeCreatedAt' ? 'selected' : ''}>좋아요 누른 날짜</option>
                        <option value="postCreatedAt" ${param.dateType eq 'postCreatedAt' ? 'selected' : ''}>게시글 작성일자</option>
                    </select>
                </div>
                <div class="col-md-8">
            ...
            <div class="row g-2 align-items-center mb-3">
                <div class="col-md-3">
                    <select name="searchType" class="form-select">
                        ...
                        <option value="postContent" ${searchDTO.searchType eq "postContent" ? "selected":""}>내용</option>
    ...
    <c:if test="${not empty likesList}">
        <table class="table table-hover text-center align-middle">
            ...
            <tbody>
            <c:forEach items="${likesList}" var="likesDTO" varStatus="status">
                <tr>
                    <td>${status.index + 1}</td>
                    <td class="text-start">
                        <a href="/post/detail?postIdx=${likesDTO.postLikeRefIdx}"
                           class="text-decoration-none text-dark">
                                ${likesDTO.postTitle}
                        </a>
                    </td>
                    <td>${likesDTO.postMemberId}</td>
                    <td>${likesDTO.postCreatedAt.toLocalDate()}</td>
                </tr>
            </c:forEach>
            </tbody>
        </table>
